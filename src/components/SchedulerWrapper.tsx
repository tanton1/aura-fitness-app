...
export default function SchedulerWrapper({ user, profile }: Props) {
  ...
  const studentContracts = useMemo(() => {
    ...
    sortedContracts.forEach((c) => {
      if (c.status === "active" && !map.has(c.studentId)) {
        const startDate = c.startDate ? new Date(c.startDate) : new Date(0);
        ...
        const endDate = c.endDate
          ? new Date(c.endDate)
          : new Date(now.getTime() + 1000 * 3600 * 24 * 365);
  ...
  const handleGenerate = () => {
    ...
    if (generatedCount === previousCount) {
      setDebugData({
        message:
          "Hệ thống không thể xếp thêm được lịch nào. Hãy xem debugSteps.",
        activeStudentsLength: activeStudentList.length,
        branchStudentsLength: branchStudents.length,
        debugSteps: result.debugSteps,
      });
      const debugLogs = result.debugSteps?.length ? `\nChi tiết Log (vài dòng đầu):\n${result.debugSteps.slice(0, 10).join('\n')}` : '';
      alert(
        `Hệ thống không thể xếp thêm được lịch nào.\n- Số học viên có HĐ active: ${activeStudentList.length}/${branchStudents.length}\n- Số buổi được xếp mới: 0\n${debugLogs}`,
      );
      console.error(
  ...
  const handleConfirmAttendance = async (
    ...
    const updatedContract = {
      ...
      status:
        contract.usedSessions + 1 >= contract.totalSessions
          ? "expired"
          : "active",
        ...
        <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800 shadow-xl">
          ...
          <StudentForm
            ...
            initialData={{
              id: user?.uid || "",
              name: profile?.name || "",
              sessionsPerWeek: currentUserStudent?.sessionsPerWeek || 3,
              availableSlots: [],
            }}
            scheduleConfig={activeScheduleConfig}
            isAdmin={profile?.role === "admin"}
                          ...
                          <div className="mt-5 pt-5 border-t border-zinc-800/50">
                            ...
                            <div className="w-full bg-zinc-900 rounded-full h-3 overflow-hidden border border-zinc-800">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, (contract.usedSessions / contract.totalSessions) * 100)}%`,
                                }}
                              >
                    ...
                    <div>
                      ...
                      <p className="text-zinc-400 flex items-center gap-1 mt-1">
                        ...
                        {currentUserStudent.branchId
                          ? "Cơ sở " + currentUserStudent.branchId
                          : "Chưa cập nhật cơ sở"}
                      </p>
                    </div>
                  </div>
